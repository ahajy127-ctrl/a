...
def _trend(c, coin, c4=None):
    ...
    if c4 and len(c4) >= 10:
        try:
            ...
            if c4_arr[-1] > c4_arr[0]:
                c4_bonus = 0.5
            elif c4_arr[-1] < c4_arr[0]:
                c4_bonus = -0.5  # PENALIZE if 4h disagrees
        except Exception:
            pass
    # REAL trend requires full alignment: s20 > s50 > s100 = strong uptrend
    if s20 > s50 > s100:
        sc = min(5, (s20 - s100) / max(abs(s100), 1) * 1000) + c4_bonus; dr = 'long'
    # Full downtrend: s20 < s50 < s100
    elif s20 < s50 < s100:
        sc = min(5, abs(s20 - s100) / max(abs(s100), 1) * 1000) + c4_bonus; dr = 'short'
    # Mixed alignment (s20 vs s50 disagree with s100) = NO TREND signal
    else:
        return None
...
def _breakout(c, coin):
    if not c or len(c) < 21: return None
    prev = c[-21:-1]; lo, hi = min(prev), max(prev); cur = c[-1]
    mom = (cur - c[-5]) / max(c[-5], 1e-9) if len(c) > 5 else 0
    sc, dr = 0, None
    if cur >= hi * 1.005:
        sc = min(5, (cur - hi) / max(hi, 1e-9) * 200)
        if mom > 0.01: sc += 2
        dr = 'long'
    elif cur <= lo * 0.995:
        sc = min(5, (lo - cur) / max(lo, 1e-9) * 200)
        if mom < -0.01: sc += 2
        dr = 'short'
